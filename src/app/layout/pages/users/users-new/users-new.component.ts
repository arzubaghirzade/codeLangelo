...
export class UsersNewComponent implements OnChanges {
  @Input() users: any;
  ...
  ngOnChanges (changes: SimpleChanges) {
    this.arrayObj = changes.users.currentValue;
    if(this.arrayObj.id){
...
createForm() {
  this.validateForm = this.fb.group({
    ...
    navigatorUrl: ['/layout/users/users-detaiils']
  });
  ...
  submitForm(value) {
    ...
    console.log(formObj);
    this.appService.updateForm('users',  this.param_id, formObj).subscribe(result => {
      this.onClose.emit();
    });
    } else {
      const formObj = { 
      ...
      this.appService.postForm('users', formObj).subscribe(result => {
        this.onClose.emit();
      });
    }
  }
